...
// ../skills/bespoke_design_system/scripts/filter-typography.ts
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--niche" && args[i + 1]) {
      niche2 = args[i + 1];
      i++;
    } else if (args[i] === "--application-type" && args[i + 1]) {
      applicationType2 = args[i + 1];
      i++;
  ...
  return { niche: niche2, applicationType: applicationType2, count: count2, category: category2 };
}
function csvRowToTypography(row, index) {
  const raw = row.raw;
...
function selectDiverse(entries, count2) {
  if (entries.length <= count2) return entries;
  const categories = [...new Set(entries.map((e) => e.category))];
  const selected = [];
  const perCategory = Math.ceil(count2 / categories.length);
  ...
  if (selected.length < count2) {
    ...
    selected.push(...shuffle(remaining).slice(0, count2 - selected.length));
  }
  return selected.slice(0, count2);
}
function filterTypography(niche2, applicationType2, count2, category2) {
  ...
  if (applicationType2) {
    ...
    if (exact.length >= count2) {
      const entries2 = exact.map((r, i) => csvRowToTypography(r, i));
      return {
        typography: selectDiverse(entries2, count2),
  ...
  if (nicheMatches.length >= count2) {
    const entries2 = nicheMatches.map((r, i) => csvRowToTypography(r, i));
    return {
      typography: selectDiverse(entries2, count2),
  ...
  if (expanded.length >= count2) {
    const entries2 = expanded.map((r, i) => csvRowToTypography(r, i));
    return {
      typography: selectDiverse(entries2, count2),
  ...
  return {
    typography: selectDiverse(entries, Math.min(count2, entries.length)),
    total_matches: all.length,
    filter_strategy: "fallback"
  };
}
var { niche, applicationType, count, category } = parseArgs();
var result = filterTypography(niche, applicationType, count, category);
var outputDir = ".design-pipeline/typography";

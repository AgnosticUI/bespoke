...
```bash
node scripts/match-niche.js --description "{user_description}"
```
...
```bash
node scripts/generate-layouts.js --niche "{NICHE}" --count 3
```
...
```bash
node scripts/filter-typography.js --niche "{NICHE}" --application-type "{APP_TYPE}" --count 3
```
...
```bash
node scripts/combine-previews.js --layouts "1,2,3" --typography "1,2,3"
```
...
```bash
node scripts/generate-palette-combinations.js --combination "combo-{BEST_COMBO_PADDED}" --niche "{NICHE}" --count 3
```
...
```bash
node scripts/generate-tokens.js --frameworks shadcn,generic,daisyui,agnosticui --palette "{BEST_PALETTE}"
```
